...
input.ax=tmp_d(:,1);   input.az=tmp_d(:,2);
input.bx=tmp_d(:,3);   input.bz=tmp_d(:,4);
input.mx=tmp_d(:,5);   input.mz=tmp_d(:,6);
input.nx=tmp_d(:,7);   input.nz=tmp_d(:,8);
%--------------------------------
...
ind = (all_probe(:,2)==0);
input.surf_elec = all_probe(ind,:);
ind = (all_probe(:,2)~=0);
input.bore_elec = all_probe(ind,:);
% 电极的索引
