...
mesh.rho(model.p2c) = model.res_param_itr;
...
if (jc_flag==2 || itr==0)
    for i = 1:input.num_mes
        jacb_mes = jac_tmp(i,:);
        geofac= input.geofac(i);
        app_res = fem.app_res_itr(i);    % 视电阻率
        mod_res = model.res_param';  % 模型电阻率（真）
        array_jac(i,:) = geofac*jacb_mes./mod_res/app_res;
    end
...
% ------------------------------------------------------------------------%
% 矩阵的预分配
% f = spalloc(mesh.num_nodes,model.num_elecs,model.num_elecs); 
% 源项
num_elecs = length(mesh.mes_node);
...
jac=zeros(input.num_mes,model.num_param);
...
for mes=1:input.num_mes  %每个实测数据
    ...
    if rec~=0
        ...
        c_kb = zeros(3,model.num_param);
    end
    im = pm(mes);
    if im~=0
        Um = reshape(aaa(im,tri2node),3,model.num_param);
    else
        Um = zeros(3,model.num_param);
    end
    in = pn(mes);
    if im~=0
        Un = reshape(aaa(in,tri2node),3,model.num_param);
    else
        Un = zeros(3,model.num_param);
    end
    jactmp = sum((c_ka-c_kb).*(Um-Un),1);  
    jac(mes,:) = reshape(jactmp,1,[]);    
end
